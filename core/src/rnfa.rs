...
pub struct NFAState {
    transitions: HashMap<Terminal, HashSet<StateId>>,
    capture_points: HashSet<CapturePoint>,
}
...
impl RecursiveNFA {
    ...
    pub fn add_transition(
        ...
        let state = self.table.get_mut(from_id).ok_or(format!(
            "'from' state {} doesn't exist in this NFA.",
            from_id
        ))?;
    ...
    pub fn get_state(&self, state_id: &StateId) -> Result<&NFAState, String> {
        if let Some(state) = self.table.get(state_id) {
            ...
            Err(format!(
                "State with id {} doesn't exist in this NFA",
                state_id
            ))
        }
    }
    ...
    pub fn set_start_states(&mut self, states: HashSet<StateId>) -> Result<(), String> {
        if let Some(state) = states.iter().find(|state| !self.is_state(*state)) {
            Err(format!(
                "state {} requested to be start state is not a state of NFA",
                state
            ))
        } else {
    ...
    pub fn set_end_states(&mut self, states: HashSet<StateId>) -> Result<(), String> {
        if let Some(state) = states.iter().find(|state| !self.is_state(*state)) {
            Err(format!(
                "state {} requested to be end state is not a state of NFA",
                state
            ))
        } else {
    ...
    pub fn get_transitions(&self) -> HashSet<(StateId, StateId, Terminal)> {
        self.table
            .iter()
            .flat_map(|(from_id, from)| from.iter().map(move |to_id| (from_id, to_id)))
            .flat_map(|(from_id, (terminal, transitions))| {
                transitions
                    .iter()
                    .map(move |to_id| (from_id, to_id, terminal))
            })
